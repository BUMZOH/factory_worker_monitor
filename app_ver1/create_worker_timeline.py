...
def save_timeline_image(target_date: str) -> Path:
    ...
    csv_path = CSV_DIR / f"worker_detection_{target_date}.csv"

    timeline_data = load_timeline_data(csv_path, target_date)

    working_minutes = calculate_working_time(csv_path, target_date)

    date_for_display = f"{target_date[:4]}-{target_date[4:6]}-{target_date[6:]}"
    title = f"Worker Detection Timeline {date_for_display} 04:00 - next day 03:59"

    image = create_timeline_image(timeline_data, title, working_minutes)

    image_path = IMAGE_DIR / f"worker_timeline_{target_date}.png"
